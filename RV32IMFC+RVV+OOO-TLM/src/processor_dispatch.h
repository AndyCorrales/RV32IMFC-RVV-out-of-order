...
inline void ProcessorOOO::dispatch(uint32_t instr, uint8_t isize) {
        ...
        if (opc == rv32i::Opcode::LUI || opc == rv32i::Opcode::AUIPC ||
            ...
            if (!vec_rs.busy && !sys_rs.busy) {
                ...
                vec_rs.vm = ((instr >> 25) & 1) != 0; // bit vm, igual que en aritmetica
                vec_rs.rob_tag = new_tag;
                vec_rs.vl = csr_vl;             // largo vectorial vigente al despachar
                vec_rs.vstart = (uint8_t)csr_vstart; // Fase 6: primer elemento a mover
                ...
                vec_rs.mop = static_cast<uint8_t>(mop);
                vec_rs.nf  = static_cast<uint8_t>((instr >> 29) & 0x7);
                vec_rs.fld = 0;
                bool unit = (mop == RVV_MOP_UNIT);
                vec_rs.lsmode = (unit && lumop == RVV_LUMOP_MASK)  ? LSM_MASK  :
                ...
                vec_rs.idx_b = w_b;
                vec_rs.sew_b = indexed ? cur_sew_bytes() : w_b;
                vec_rs.vs2 = rs2i;              // indices (indexado) -- banco vectorial
                vec_rs.s1 = read_operand(rs1i); // direccion base, banco entero
                if (mop == RVV_MOP_STRIDED) {
                    vec_rs.s2 = read_operand(rs2i); // paso en BYTES, banco entero
                } else {
                    vec_rs.s2.ready = true; vec_rs.s2.val = 0; vec_rs.s2.tag = 0;
                }
                RobEntry& e = rob[new_tag];
            ...
            if (supported) {
                ...
                if (!vec_rs.busy && !sys_rs.busy) {
                    ...
                    vec_rs.funct6 = f6; vec_rs.funct3 = f3; vec_rs.vm = vm;
                    vec_rs.vcat = vcat; vec_rs.vs1_field = rs1i;
                    vec_rs.rob_tag = new_tag;
                    vec_rs.vl = csr_vl;          // largo vectorial vigente al despachar
                    vec_rs.vstart = (uint8_t)csr_vstart; // Fase 6
                    vec_rs.sew_b = cur_sew_bytes(); // ancho de elemento vigente
                    ...
                    if (f3 == RVV_FUNCT3_OPIVX || f3 == RVV_FUNCT3_OPMVX) {
                        vec_rs.s1 = read_operand(rs1i); // x[rs1], espera al CDB si hace falta
                    } else if (f3 == RVV_FUNCT3_OPIVI) {
                        // imm[4:0] con signo, en el mismo campo que rs1
                        int32_t simm = static_cast<int32_t>(rs1i << 27) >> 27;
                        vec_rs.s1.ready = true; vec_rs.s1.val = static_cast<uint32_t>(simm);
                        vec_rs.s1.tag = 0;
                    } else {
                        vec_rs.s1.ready = true; vec_rs.s1.val = 0; vec_rs.s1.tag = 0;
                    }
                    RobEntry& e = rob[new_tag];
                    e.valid = true; e.is_store = false; e.dest_is_fp = false;
