    ...
    bool can_dispatch = false;
        ...
        if (opc == rv32i::Opcode::LUI || opc == rv32i::Opcode::AUIPC ||
            ...
            if (!vec_rs.busy && !sys_rs.busy) {
                ...
                vec_rs.vm = ((iw >> 25) & 1) != 0; // bit vm, igual que en aritmetica
                vec_rs.rob_tag = new_tag;
                vec_rs.vl = csr_vl;    // largo vectorial vigente en este punto del programa
                vec_rs.vstart = csr_vstart; // Fase 6: primer elemento a mover
                ...
                vec_rs.idx_b = w_b;
                vec_rs.sew_b = indexed
                    ? ap_uint<3>((8u << ((csr_vtype.to_uint() >> 3) & 0x7)) / 8u)
                    : w_b;
                vec_rs.vs2 = rs2;              // indices (indexado) -- banco vectorial
                vec_rs.s1 = read_operand(rs1); // direccion base, banco entero
                if (mop == rvv::MOP_STRIDED) {
                    vec_rs.s2 = read_operand(rs2); // paso en BYTES, banco entero
                } else {
